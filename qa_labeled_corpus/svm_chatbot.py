...
# コーパスの読み込み
def load_labeled_corpus(file_path):
...
# ユーザー入力に基づく応答生成
def get_response(user_input, vectorizer, questions, answers):
    # ユーザー入力をベクトル化
    user_input_vector = vectorizer.transform([user_input])
    
    # 類似度の計算
    similarities = cosine_similarity(user_input_vector, vectorizer.transform(questions))
...
def main():
    ...
    # ラベル付きコーパスの読み込み
    qa_pairs = load_labeled_corpus('qa_labeled_corpus.txt')
    questions = [pair[1] for pair in qa_pairs]
    answers = [pair[2] for pair in qa_pairs]
    ...
    while True:
        ...
        # 応答を取得して表示する
        response = get_response(user_input, vectorizer, questions, answers)
